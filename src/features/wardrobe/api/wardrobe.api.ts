...
export const wardrobeApi = {
  getMyWardrobeItems: async (): Promise<WardrobeItemRes[]> => {
  ...
  batchUploadWardrobeItems: async (data: BatchCropWardrobeItemsReq): Promise<WardrobeItemRes[]> => {
    const res = await axiosInstance.post<APIResponse<WardrobeItemRes[]>>('/wardrobe-items/batch-upload', data);
    return res.data.data || [];
  },

  initClosetFromCatalog: async (data: InitClosetFromCatalogReq): Promise<WardrobeItemRes[]> => {
    const res = await axiosInstance.post<APIResponse<WardrobeItemRes[]>>('/wardrobe-items/catalog-init', data);
    return res.data.data || [];
  },
  ...
  cloneWardrobeItem: async (id: string, data: CloneWardrobeItemReq): Promise<WardrobeItemRes[]> => {
    const res = await axiosInstance.post<APIResponse<WardrobeItemRes[]>>(`/wardrobe-items/${id}/clone`, data);
    return res.data.data || [];
  },

  updateWardrobeItem: async (id: string, data: UpdateWardrobeItemReq): Promise<WardrobeItemRes> => {
    const res = await axiosInstance.put<APIResponse<WardrobeItemRes>>(`/wardrobe-items/${id}/manual-classify`, data);
    return res.data.data!;
  },
  ...
  searchWardrobeItems: async (query: string): Promise<SearchWardrobeItemRes[]> => {
